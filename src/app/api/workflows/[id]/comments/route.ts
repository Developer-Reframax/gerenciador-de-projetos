...
// GET - Listar comentários do workflow
export async function GET(
  ...
  try {
    ...
    // Buscar comentários do workflow
    ...
    const { data: authors, error: authorsError } = await supabase
      ...
      .in('id', authorIds.filter((id): id is string => id !== null))
    ...
    console.error('Erro na API de comentários do workflow:', error)
    return NextResponse.json(
      { error: 'Erro interno do servidor' },
      { status: 500 }
    )
