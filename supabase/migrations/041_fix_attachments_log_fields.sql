-- Fix attachments log fields mapping
-- The function was trying to access NEW.name but attachments table has 'filename' and 'original_filename'
...
BEGIN
    ...
    -- Se não conseguiu determinar o usuário, usar um UUID padrão
    ...
    CASE TG_OP
        WHEN 'INSERT' THEN
            CASE TG_TABLE_NAME
                WHEN 'attachments' THEN
                    log_description := 'Criou ' || get_entity_name(TG_TABLE_NAME) || ': ' || COALESCE(NEW.original_filename, NEW.filename, 'Novo arquivo');
                WHEN 'comments' THEN
        ...
        WHEN 'UPDATE' THEN
            CASE TG_TABLE_NAME
                WHEN 'attachments' THEN
                    log_description := 'Atualizou ' || get_entity_name(TG_TABLE_NAME) || ': ' || COALESCE(NEW.original_filename, NEW.filename, 'Arquivo');
                WHEN 'comments' THEN
        ...
        WHEN 'DELETE' THEN
            CASE TG_TABLE_NAME
                WHEN 'attachments' THEN
                    log_description := 'Excluiu ' || get_entity_name(TG_TABLE_NAME) || ': ' || COALESCE(OLD.original_filename, OLD.filename, 'Arquivo');
                WHEN 'comments' THEN
...
EXCEPTION
    WHEN OTHERS THEN
        ...
        RAISE NOTICE 'Erro ao criar log: %', SQLERRM;
        RETURN COALESCE(NEW, OLD);
END;
